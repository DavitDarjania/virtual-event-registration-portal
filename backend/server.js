...
async function initializeEvents() {
  try {
    ...
    await fs.writeFile(EVENTS_FILE, JSON.stringify({ events: initialEvents, registrations: [] }, null, 2));
  }
}
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const event = jsonData.events.find(e => e.id === registrationData.eventId);
    if (!event) {
